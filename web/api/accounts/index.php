...
switch ($type) {
    case 'signup':
        ...
        if (!empty($input['email']) && !empty($input['password']) && !empty($input['username'])) {
            ...
            $username = trim(strtolower(addslashes($input['username'])));
            ...
            $checkQuery = "SELECT * FROM accounts WHERE `username` = '{$username}' order by id desc Limit 1";
            $queryRun = mysql_query($checkQuery);
            ...
            // Insert new user
            $query = "INSERT INTO accounts (email, `username` ,`password`, `added`, `email_hash`, `token_hash`, `ipaddress`) VALUES ('$email', '$username'  ,'$password', '$added', '$email_hash', '$token_hash', '$ipaddress')";
    ...
    case 'get_account':
        ...
        if (!empty($input['token'])) {
            ...
            $checkQuery = "SELECT * FROM accounts WHERE `token_hash` = '{$token}' order by id desc Limit 1";
            $queryRun = mysql_query($checkQuery);
    ...
    case 'forgot_password':    
            ...
            if (!empty($input['email'])) {
                ...
                <a href='https://foodie.app/reset-password/?email={$email}'>Reset Password</a><br><br>
                Thanks,<br>Foodie Team";
