...
// Fetch one record
$stmt = mysql_query("SELECT id,post_url FROM ext_post_qc WHERE (`post_embed` is null OR `post_embed` = '') AND `ue_done`=1 LIMIT 10");
...
while ($row = mysql_fetch_array($stmt)) {
    ...
    mysql_query("UPDATE ext_post_qc SET `post_embed`='".$embedding['response']."', `ft_ready`=1 WHERE `id`={$id}");
