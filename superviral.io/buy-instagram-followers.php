...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/buy-instagram-followers.html');

}else{
	$tpl = file_get_contents('us/buy-instagram-followers.html');
    
}

// $tpl = file_get_contents('buy-instagram-followers-3.html');

//if($_GET['rabban']=='true')$tpl = file_get_contents('buy-instagram-followers-4.html');
...
if($_GET['p']=='1')$q = mysql_query("SELECT * FROM `packages` WHERE `brand`='sv' AND `type` = 'followers' AND `premium` = '1' AND socialmedia = 'ig' ORDER BY `amount` ASC");
if($_GET['p']=='2')$q = mysql_query("SELECT * FROM `packages` WHERE `brand`='sv' AND `type` = 'followers' AND `premium` = '2' AND socialmedia = 'ig' ORDER BY `amount` ASC");
...
$qp = mysql_query("SELECT * FROM `packages` WHERE `brand`='sv' AND `type` = 'followers' AND `premium` = '1' AND socialmedia = 'ig' ORDER BY `amount` ASC");
...
while ($info = mysql_fetch_array($qp)) {
    ...
    if ($info['id'] == 103) $decimalinc = '.' . $decimal . '';
    if ($info['id'] == 202) {
        $bestPackageClass = 'best';
...
$q = mysql_query("SELECT * FROM `reviews` WHERE `brand`='sv' AND `type` = 'ig-followers' AND `approved` = '1' AND `country` = '{$locas[$loc]['sdb']}' ORDER BY `id` DESC");
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `brand`='sv' AND `country` = '{$locas[$loc]['sdb']}' AND `page` IN ('buy-instagram-followers','global')");
