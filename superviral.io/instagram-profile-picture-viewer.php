...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/instagram-profile-picture-viewer.html');
}else{
	$tpl = file_get_contents('us/instagram-profile-picture-viewer.html');
}
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `country` = '{$locas[$loc]['sdb']}' AND `page` IN ('instagram-pp-viewer','global')");
