...
$body = file_get_contents('order4-5.html');
...
if($orderinfo['packagetype'] == 'views' || $orderinfo['packagetype'] == 'likes' && $socialmedia == 'ig'){
	...
	if( $show_tpl_b ){$body = file_get_contents('order4-5.html');}
...
if(empty($orderinfo['contactnumber'])){
	...
	$checkforcontactnumberq = mysql_query("SELECT * FROM `orders` WHERE 
		`brand` = 'sv' AND 
		`askednumber` = '1' AND 
		  (MATCH(`emailaddress`) AGAINST('\"{$info['emailaddress']}\"' IN NATURAL LANGUAGE MODE) 
			OR MATCH(`igusername`) AGAINST('\"{$info['igusername']}\"' IN NATURAL LANGUAGE MODE) ) 
...
$mainctabtnoption = '<a class="color4 btn dshadow" target="_BLANK" style="text-align:center;margin-top: 10px!important;padding: 9px;font-size: 16px;border-radius: 21px !important;" href="/'.$loclinkforward.'account/dashboard/">View Order & Dashboard</a>';

$mainctabtnoption = '<a onclick="ga(\'send\', \'event\', \'View\', \'Click\', \'Trackingpage\',\'1\');" class="color4 btn dshadow" target="_BLANK" style="text-align:center;margin-top: 10px!important;padding: 9px;font-size: 16px;border-radius: 21px !important;" href="/'.$loclinkforward.'track-my-order/{order_session}/{ordernumber}">{trackingdetailscta}</a>';
...
if(mysql_num_rows($fetchuseridq)=='1'){
	...
	if($is_private == 'Public'){
		$dispOnProfileStatus = 'display:none';
	}
}
...
if($showaccountsignup==1 && $accountExist == 0){
	$tpl = str_replace('<body', '<body onload="signupaccount();"', $tpl);}else{
...
$tpl = str_replace('{dispOnProfileStatus}', $dispOnProfileStatus, $tpl);  
...
sendCloudwatchData('Superviral', 'order-finish', 'UserFunnel', 'user-funnel-order-finish-function', 1);
...
sendCloudwatchData('Superviral', 'page-load-order-finish', 'PageLoadTiming', 'page-load-order-finish-function', number_format($execution_time_sec, 2));
