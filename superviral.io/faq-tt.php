...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/faq-tt.html');
}else{
	$tpl = file_get_contents('us/faq-tt.html');
}
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `brand`='sv' AND ((`country` = '{$locas[$loc]['sdb']}' AND `page` = 'faq-tt') OR (`country` = '{$locas[$loc]['sdb']}' AND `page` = 'global'))");
