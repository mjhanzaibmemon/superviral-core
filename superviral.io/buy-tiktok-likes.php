...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/buy-tiktok-likes.html');
	if($_GET['test']=='1')$tpl = file_get_contents('uk/test/buy-tiktok-likes.html');
	elseif($_GET['split'] == 'b')
        $tpl = file_get_contents('uk/split-test/buy-tiktok-likes-b.html');
}else{
	$tpl = file_get_contents('us/buy-tiktok-likes.html');
	if($_GET['test']=='1')$tpl = file_get_contents('us/test/buy-tiktok-likes.html');
	elseif($_GET['split'] == 'b'){
		$tpl = file_get_contents('us/split-test/buy-tiktok-likes-b.html');
		$ipinfo = getUserIpInfo($ipaddress);
		$flagShowCall = "";
		if(!empty($ipinfo['country'])){
...
$q = mysql_query("SELECT * FROM `packages` WHERE `brand`='sv' AND socialmedia = 'tt' AND `type` = 'likes' AND `premium` = '0' ORDER BY `amount` ASC");
if($_GET['p']=='1')$q = mysql_query("SELECT * FROM `brand`='sv' AND socialmedia = 'tt' AND `packages` WHERE `type` = 'likes' AND `premium` = '1' ORDER BY `amount` ASC");
if($_GET['p']=='2')$q = mysql_query("SELECT * FROM `brand`='sv' AND socialmedia = 'tt' AND `packages` WHERE `type` = 'likes' AND `premium` = '2' ORDER BY `amount` ASC");
...
// 				              		<li><span class="tick"></span>{packagetick7}</li>
// 				              		<li><span class="tick"></span>{packagetick2}</li>
// 									<li><span class="tick"></span>{packagetick8}</li>
...
$q = mysql_query("SELECT * FROM `reviews` WHERE `brand`='sv' AND `type` = 'tt-likes' AND `approved` = '1' AND `country` = '{$locas[$loc]['sdb']}' ORDER BY `id` DESC");
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `brand`='sv' AND (`country` = '{$locas[$loc]['sdb']}' AND `page` IN ('buy-tiktok-likes','global'))");
...
$lowestPkgq = mysql_query("SELECT * FROM `packages` WHERE `brand`='sv' AND socialmedia = 'tt' AND `type` = 'likes' ORDER BY `price` ASC limit 1");
