...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/privacy-policy.html');
}else{
	$tpl = file_get_contents('us/privacy-policy.html');
}
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `country` = '{$locas[$loc]['sdb']}' AND `page` IN ('privacy-policy','global')");
