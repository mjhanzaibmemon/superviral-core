...
$tpl = file_get_contents('turn-off-post-notifs.html');
...
if((!empty($_POST['submit']))&&(!empty($_POST['id']))){
		...
		if(!empty($_POST['disable'])){
				...
				$data = mysql_query("UPDATE `users` SET 
				                                    `notification_disabled` = 1
				                                    WHERE `md5` = '$id' LIMIT 1");

				if($data){
				    	$domessage = '<div class="message">You\'ve successfully disabled post notifications from Superviral. We hope to see you again.</div>';
...
$q = mysql_query("SELECT * FROM `users` WHERE `md5` = '$id' LIMIT 1");
...
$btn = '<input type="submit" name="submit" value="Disable post notifications" class="btn btn3 showmo disablebtn">';
...
$btn = '<input type="submit" name="submit" value="Enable post notifications" class="btn btn3 color4 enablebtn">';
