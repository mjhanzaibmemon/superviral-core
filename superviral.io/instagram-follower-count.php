...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/instagram-follower-count.html');
}else{
	$tpl = file_get_contents('us/instagram-follower-count.html');
}
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `country` = '{$locas[$loc]['sdb']}' AND `page` IN ('instagram-follower-count','global')");
