...
if($locas[$loc]['sdb'] == 'uk'){
	$tpl = file_get_contents('uk/buy-instagram-views.html');
	if($_GET['test']=='1')$tpl = file_get_contents('uk/test/buy-instagram-views.html');
	elseif($_GET['split'] == 'b')
        $tpl = file_get_contents('uk/split-test/buy-instagram-views-b.html');
}else{
	$tpl = file_get_contents('us/buy-instagram-views.html');
	if($_GET['test']=='1')$tpl = file_get_contents('us/test/buy-instagram-views.html');
	elseif($_GET['split'] == 'b'){
		$tpl = file_get_contents('us/split-test/buy-instagram-views-b.html');
		$ipinfo = getUserIpInfo($ipaddress);
		$flagShowCall = "";
		if(!empty($ipinfo['country'])){
...
// $tpl = file_get_contents('buy-instagram-views-4.html');
if($_GET['rabban']=='true')$tpl = file_get_contents('buy-instagram-views-4.html');

$q = mysql_query("SELECT * FROM `packages` WHERE `type` = 'views' AND `premium` = '0' AND `brand`='sv' AND socialmedia = 'ig' ORDER BY `amount` ASC");

$maxPrice = 0;
$minPrice = 1000;
$countPackage = mysql_num_rows($q);

if($_GET['p']=='1')$q = mysql_query("SELECT * FROM `packages` WHERE `type` = 'views' AND `premium` = '1' AND `brand`='sv' AND socialmedia = 'ig' ORDER BY `amount` ASC");
if($_GET['p']=='2')$q = mysql_query("SELECT * FROM `packages` WHERE `type` = 'views' AND `premium` = '2' AND `brand`='sv' AND socialmedia = 'ig' ORDER BY `amount` ASC");
...
if($info['id']==12)$decimalinc = '.'.$decimal.'';
if($info['id'] == 14){$bestPackageClass = 'best'; $popular_class = 'popular';}else{$bestPackageClass = ''; $popular_class = '';}
...
$q = mysql_query("SELECT * FROM `reviews` WHERE `brand`='sv' AND `type` = 'ig-views' AND `approved` = '1' AND `country` = '{$locas[$loc]['sdb']}' ORDER BY `id` DESC");
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `brand`='sv' AND ((`country` = '{$locas[$loc]['sdb']}' AND `page` = 'buy-instagram-views') OR (`country` = '{$locas[$loc]['sdb']}' AND `page` = 'global'))");
