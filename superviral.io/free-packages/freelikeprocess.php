...
$uniquepaymentid = 'FREE-' .$info['igusername'] .'#'.$info['id'] ;
...
$updatefulfill = mysql_query("UPDATE `freetrial` SET `orderid`='$uniqueorderinsertid',`username` = '{$info['igusername']}' WHERE `emailaddress` = '{$info['emailaddress']}' ORDER BY id desc LIMIT 1");


mysql_query("UPDATE `order_session` SET `done` = '1',`payment_attempts` = '0' WHERE `order_session` = '$ordersession' LIMIT 1");

mysql_query("UPDATE `freetrial` SET `done` = '1' WHERE `emailaddress` = '{$info['emailaddress']}'");

$updateuser = mysql_query("UPDATE `users` SET `monthlyfreelikes` = '1' WHERE `emailaddress` = '{$info['emailaddress']}' ");
///////////////////////////
...
if (!empty($searchuserpast['contactnumber'])) {
    $addcontactnumber = ", `askednumber` = '2', `contactnumber` = '{$searchuserpast['contactnumber']}' ";
} else {
    $addcontactnumber = "";
}


$updateorder = mysql_query("UPDATE `orders` SET `choose_comments` = '$chooseCommentsql', `chooseposts` = '$choosepostsql',`added` = '$added' $addcontactnumber WHERE `order_session` = '{$info['order_session']}' LIMIT 1");
...
if ($orderinserted) {
    ...
    if ($dontredirectwebhook !== 1) {
        echo "<script>window.top.location.href = '/free-package/processing/?freelikesmsg=". $ordersession ."';</script>"; 

        // header('Location: /' . $locas[$loc]['order'] . '/' . $locas[$loc]['order4'] . '/' . $redirectq12);
