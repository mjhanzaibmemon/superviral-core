...
// die('Unfortunately, due to high demand we\'re unable to provide free likes for today. Get Tiktok likes <a href="https://superviral.io/buy-tiktok-likes/">here</a>.');
...
if (!empty(addslashes($_POST['submitForm']))&&($info['done']=='0')) {
    ...
    if (empty($error1)) {
        ...
        $username = str_replace('@', '', $username);
        $username = str_replace('https://tiktok.com/', '', $username);
        $username = str_replace('tiktok.com/', '', $username);
        ...
        $checkq = mysql_query("SELECT * FROM `packages` WHERE `type` = 'freelikes' AND socialmedia = 'tt' AND brand ='sv' LIMIT 1");
...
$tpl = file_get_contents('tt-freelikes.html');
