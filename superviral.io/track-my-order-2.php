...
$q = mysql_query("SELECT * FROM `packages` WHERE `amount` = '$packageamount1' OR `amount` = '$packageamountupsell1' LIMIT 1");
...
if($info['estdeliverytime']=='0'){
  ...
  $fetchordersessionq = mysql_query("SELECT * FROM `order_session` WHERE `order_session` = '{$info['order_session']}' LIMIT 1");
  $fetchordersession = mysql_fetch_array($fetchordersessionq);

  $getpackageinfoq = mysql_query("SELECT * FROM `packages` WHERE `id` = '{$fetchordersession['packageid']}' LIMIT 1");
...
if($hours=='1'||$hours=='0'){$time= $minutes.' minutes';}else{
...
if($days==0){unset($time); $time= $hours.' hours';}
...
if($info['refund']=='2'){$deliverystatus = 'Refund issued to the card<br>ending with '.$info['lastfour'].' for this order';
...
if(empty($info['contactnumber'])){$contactnumber = '<a class="thehref" onclick="signup();return false;" href="#">Add phone number for free order update</a>';$textnotifsbtn = '<a onclick="signup();return false;" href="#" class="btn color3 dshadow" style="margin-top:15px;">Get Free Order Notifications</a>';}
  else{$contactnumber = '<a class="thehref" onclick="signup();return false;" href="#">'.$info['contactnumber'].' (change)</a>';}

if(($info['freelikes']=='0')&&($info['packagetype']=='followers'))$freelikesbtn = '<a onclick="signup2();return false;" href="#" class="btn color3 dshadow" style="margin-top:10px;">Get Free Instagram Likes</a>';
...
$q = mysql_query("SELECT * FROM `orders` WHERE `added` BETWEEN '$threedaysago' AND '$yesterday'");
$orderamount = round(mysql_num_rows($q) * 6.64);
...
$tpl = str_replace('{progressbarstyle}', $progressbarstyle, $tpl);
