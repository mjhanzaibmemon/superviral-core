...
if (!empty($ipData['country']) && strtoupper($ipData['country']) === "US") {
    header("Location: /account/al-checkout/$id");
    exit;
}else{
    $currentPath = $_SERVER['REQUEST_URI'];
  if (
    strpos($currentPath, '/account/checkout/') !== false &&
    strpos($currentPath, '/uk/account/checkout/') === false
  ) {

    // Perform redirect
    header("Location: /uk/account/checkout/$id");
    exit;
  }
}
...
if (!empty($submit)) { //A PAYMENT METHOD HAS BEEN SELECTED and this is a user's actions
  ...
  if($userBlacklist== 0){
    if (!empty($_POST['username'])) {
      ...
      mysql_query("UPDATE `automatic_likes_session` SET `payment_onetime_token` = '{$onetimetoken}' WHERE `id` = '{$info['id']}' LIMIT 1");
      ...
      try {
        ...
        $merchant_order_id = date('Ymdhis') . rand(10000, 99999);
        ...
        $method_notification_url = $siteDomain . "/" . $loclinkforward . "account/account-order3-acquired-notification.php";
        ...
        if ($card_response["enrolled"] && !empty($card_response["server_trans_id"])) {
          ...
          $contact_url = $siteDomain . "/" . $loclinkforward . "account/account-order3-acquired-contact.php";
  
          $challenge_url = $siteDomain . "/" . $loclinkforward . 'account/account-order3-acquired-3dv2.php?loc=' . $loc . '&redirectid=' . $info['order_session'];
        ...
        if (in_array($response['response_code'], array(503))) { // For 3Dv2 return 503
          ...
          $threeDSSessionData["order_session"] = $id;
  
          $threeDSSessionData["expdate"] = $expdate2;
          ...
          $termurl = $siteDomain . "/" . $loclinkforward . 'account/account-order3-acquired-3dv1.php?loc=' . $loc . '&redirectid=' . $info['order_session'];
...
try {
  ...
  // empty existing cookie
  if (isset($_COOKIE['ResponseMessage'])) {

    setcookie('ResponseMessage', '', time() - 999999, '/', $_SERVER['SERVER_NAME']);
    setcookie('paymentId', '', time() - 999999, '/', $_SERVER['SERVER_NAME']);
    setcookie('expdate', '', time() - 999999, '/', $_SERVER['SERVER_NAME']);
  }
  ...
  if ($status == 'approved') {
    ...
    $payment_amount = $al_package_price;
