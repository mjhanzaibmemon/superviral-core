...
//CHECK IF THIS ACCOUNT HAS ATLEAST ONE ORDER
// $q = mysql_query("SELECT * FROM `orders` WHERE `account_id` = '{$userinfo['id']}' AND `price` != '0.00' AND `brand`='sv'");
// 	if(mysql_num_rows($q) < 3)$dontdisplayfreealbox = 1;
...
if($dontdisplayfreealbox ==0){
$q = mysql_query("SELECT * FROM `automatic_likes` WHERE `emailaddress` = '{$userinfo['email']}' LIMIT 1 ");
if(mysql_num_rows($q)==1){$dontdisplayfreealbox = 1;}
...
if($_GET['orderlogin']=='true')$message1 = '<div class="emailsuccess">Logged in successfully! Go back to your order tab to resume order!</div>';
...
if(mysql_num_rows($q)==0){$orders = 'Once you\'ve made an order while logged in, it will show up here.';}
		...
		while($info = mysql_fetch_array($q)){
			...
			if($info['fulfilled']=='0'){
				...
				$status = 'delivered '.date("D jS F Y",$info['fulfilled']);
			}
							...
							if(($info['packagetype'] !== "freefollowers")&&($info['packagetype'] !== "freelikes"))	{
									...
									<form class="mobileshidethis" method="post" action="/receipt-pdf-generator.php" >
										...
										<input name="orderPrice" type="hidden" value="'. number_format(floatval($info['price']/100), 2) .'">
										<input name="packageType" type="hidden" value="'. $info['packagetype'] .'">
										<input name="premium" type="hidden" value="'. $packageinfo['premium'] .'">
										<input name="socialmedia" type="hidden" value="'. $info['socialmedia'] .'">
										<img class="receipt" onclick="$(this).closest(\'form\').submit();" src="/imgs/bill.png" style="height: 26px; cursor: pointer">
          ...
          <div class="history-stats '.$class.'">
            <h2>+'.$info['amount']. ' ' .$SM .' '.$info['packagetype'].'
...
$findsubcriptonsq = mysql_query("SELECT * FROM `automatic_likes` WHERE `emailaddress` = '{$userinfo['email']}' AND `brand`='sv' ORDER BY `id` DESC");
...
if(mysql_num_rows($findsubcriptonsq)!==0){
	...
	while($subsinfo = mysql_fetch_array($findsubcriptonsq)){
		...
		if($subsinfo['cancelbilling']=='3'){
			$additionalstatus = ' (expires on '.date("d/m/Y",$subsinfo['expires']).')'; 

			if($subsinfo['expires'] < time()){
				$status ="";
			}
		}
...
$autolikesonaccount .= '
       <div class="subscriptions">
            ...
            <div class="substitle">
              ...
              <a href="/'.$loclinkforward.'account/edit/'.$subsinfo['md5'].'" class="btn btn3 savingcardbtn">Edit auto likes</a>
			  <button type="button" class="savingcardbtn" style="background:none;border:none!important;outline:0;height:initial;padding-right:0;" onclick="togglePopup('. $subsinfo['id'] .')">
				  <img src="/imgs/bill.png" style="height:26px;width:26px; cursor: pointer" > 
			  </button>
			  </div>
			</div>
          </div>';
...
// 		$checkUserExistQuery = mysql_query("SELECT username from account_usernames where account_id ='$accountId' order by id desc limit 1");
// 		$checkUserExistData = mysql_fetch_array($checkUserExistQuery);
// 		$userName = $checkUserExistData['username'];
// 	}
// }else{

// 	$checkUserExistQuery = mysql_query("SELECT username from account_usernames where id='$getUserid'");
// 	$checkUserExistData = mysql_fetch_array($checkUserExistQuery);
// 	$userName = $checkUserExistData['username'];
// }
...
// 	$checkUserQuery = "SELECT * FROM account_usernames WHERE username = '$userName' and active = 1";
...
// $checkViewedQuery = mysql_query("SELECT 1 from checkusers where ig_username='$userName' AND viewed = 0");
...
// 		if($_GET['orderlogin']!=='true')header('Location: /'.$loclinkforward.'account/load-dashboard/');
...
// 															WHERE cu.ig_username = '$userName' AND cun.unavailable = 0 ORDER BY cu.`id` DESC limit 1");
...
// $checkQuery = mysql_query("SELECT 1 from checkusers_now where ig_username='$userName' AND unavailable = 1"); //check for unavailable
...
//   from checkusers
//   WHERE DATE(FROM_UNIXTIME(added)) BETWEEN date_add(curdate(), interval -6 day) AND CURDATE() AND ig_username = '$userName'
...
// 										FROM ig_dp	
// 															WHERE igusername = '$userName' ORDER BY `id` DESC limit 1");
...
// 							WHERE ig.igusername ='$userName' group by ig.shortcode order by ig.added_on_instagram desc limit 12";	
...
// $userList ="";
// $userListQuery = "SELECT * FROM account_usernames WHERE account_id = $accountId and active = 1";
