...
if($getUserid == "" || $getUserid == null) {
	...
	if($userName == "" || $userName == null) {
		$checkUserExistQuery = mysql_query("SELECT username from account_usernames where account_id ='$accountId' AND `brand`='sv' order by id desc limit 1");
...
if($userinfo['viewed_dashboard'] == 0){
	...
	mysql_query("UPDATE accounts SET viewed_dashboard = 1 WHERE `id` = '{$userinfo['id']}' ");
}

$ViewedQuery = mysql_query("UPDATE checkusers SET viewed = 1 WHERE ig_username='$userName' AND `brand`='sv'");

$data = mysql_query("UPDATE post_notif_schedule SET
													email_sent = '2'
													WHERE account_id = '$accountId' AND email_sent = '0' AND `brand`='sv' LIMIT 5");
