...
if((isset($error1))||(isset($error2))||(isset($error3))||(isset($error0))){
	...
	$checkothercardsq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `approved` = '1' ");
...
if(!empty($_GET['deleteid'])){
	...
	$fetchdeletecardq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$deleteid'  LIMIT 1");
...
if(!empty($_GET['primaryid'])){
	...
	SET `primarycard` = '1'
	WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$primaryid' 
		LIMIT 1");

	$fetchprimarycardq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$primaryid'  LIMIT 1");
...
$findcardsq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `approved` = '1' ORDER BY `id` DESC");
...
$findsubcriptonsq = mysql_query("SELECT * FROM `automatic_likes` WHERE `account_id` = '{$userinfo['id']}' ORDER BY `id` DESC");
...
if(mysql_num_rows($findsubcriptonsq)!==0){
	...
	while($subsinfo = mysql_fetch_array($findsubcriptonsq)){
		...
		$fetchtransactionsq = mysql_query("SELECT * FROM `automatic_likes_billing` WHERE `auto_likes_id` = '{$subsinfo['id']}' ");
