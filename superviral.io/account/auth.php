...
if(isset($_COOKIE['plus_id']) && isset($_COOKIE['plus_token'])) {
    ...
    if($num_rows !== 1){//no match found, the combination of the email hash and token hash isn't found
		...
		header("location: /".$loclinkforward.$locas[$loc]['login']."/");
	    exit;
	...
	if(
		...
		$searchontimetokenq = mysql_query("SELECT * FROM `automatic_likes_session` WHERE `payment_onetime_token` = '$onetimetokenpost' AND `order_session` = '$alcheckoutid22' AND `payment_onetime_token_active` = '1' LIMIT 1");
		...
		$result = mysql_query("SELECT * FROM `accounts` WHERE `id` = '{$searchontimetokeninfo['account_id']}' LIMIT 1");
		$userinfo = mysql_fetch_array($result);
		$num_rows = mysql_num_rows($result);
	    if($num_rows !== 1){//no match found, the combination of the email hash and token hash isn't found
	    ...
	    mysql_query("UPDATE `automatic_likes_session` SET `payment_onetime_token_active` = '0' WHERE `payment_onetime_token` = '$onetimetokenpost' AND `order_session` = '$alcheckoutid22' LIMIT 1");
