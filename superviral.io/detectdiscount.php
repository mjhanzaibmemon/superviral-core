...
if(empty($discountid)){$discountid = addslashes($_GET['discount']);}

$discountq = mysql_query("SELECT * FROM `discounts` WHERE `md5` = '$discountid' LIMIT 1");

if(mysql_num_rows($discountq)=='1'){

mysql_query("UPDATE `discounts` SET `views` = `views` + 1 WHERE `md5` = '$discountid' LIMIT 1");
...
$discountnotiffinish = '<div style="border:1px solid green;padding:10px;margin-bottom:30px;">Order complete! You\'ve received 10% Off! Remember, you can grow your Instagram account, by an UNLIMITED AMOUNT before this discount expires. Take full advantage! This discount is for special customers Only.<br>
<a class="btn dshadow color3" style="width:initial;display:inline-block;margin:0!important;margin-top:20px!important" target="_BLANK" href="/buy-instagram-followers/" title="Buy Instagram Followers">BUY MORE FOLLOWERS »</a>
</div>';

$discounturl = '&discount='.$discountid;
