...
function setnewcookie($msg,$country,$user_ip, $id){
        ...
        global $order_session;
			...
			$insert = mysql_query("INSERT INTO `order_session` SET `country` = '{$country}',`order_session`='$order_session',`packageid` = '$id',`ipaddress` = '{$user_ip}',`igusername`='',`emailaddress`='',`done`='0',`added`='$added',`unsubscribe`='0',`abandonedemail`='0',`freefollowers`='0',`chooseposts` = '',`payment_creq_crdi` = '', brand= 'sv'");
...
if(empty($_COOKIE['ordersession'])) {
		...
		setnewcookie('',$country,$user_ip, $id);
...

{
		...
		if(mysql_num_rows($checkq)=='0'){setnewcookie('',$country,$user_ip, $id);die('ORDER SESSION NOT FOUND');}
