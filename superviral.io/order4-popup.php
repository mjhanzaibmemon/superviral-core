...
if($_GET['close']=='true'){
  ...
  sendCloudwatchData('Superviral', 'popup-not-want-benefit', 'OrderFinish', 'user-popup-not-want-benefit-function', 1);

  mysql_query("UPDATE `orders` SET  `brand`='sv', `noaccount` = '1' WHERE `id` = '$id' AND `order_session` = '$order_session' LIMIT 1");
...
$q = mysql_query("SELECT * FROM `orders` WHERE `brand`='sv' AND `id` = '$id' AND `order_session` = '$order_session' LIMIT 1 ");
...
$checkifemailexistsq = mysql_query("SELECT * FROM `accounts` WHERE `brand`='sv' AND `email` = '$emaillow' LIMIT 1");
...
if($modalshow=='existing'){//ACCOUNT HAS PREVIOUSLY BEEN CREATED
      ...
      if(!empty($_POST['submit'])){
              ...
              if((!empty($_POST['emailaddress']))&&($_POST['emailaddress'])){
                ...
                $searchaccountsq = mysql_query("SELECT * FROM `accounts` WHERE `brand`='sv' AND `email` = '$searchemailaddress' LIMIT 1");
                  ...
                  else
                {
                  ...
                  if(password_verify($searchpassword, $accountinfo['password'])){
                    //LOG CUSTOMER IN


                    // set plus_id cookie
                    setcookie('plus_id', $accountinfo['email_hash'], time() + (86400 * 365), "/","superviral.io"); // 86400 = 1 day
                    // set plus_token cookie
                    setcookie('plus_token', $accountinfo['token_hash'], time() + (86400 * 365), "/","superviral.io"); // 86400 = 1 day

                    //UPDATE THIS ORDER WITH ACCOUNT ID AND `noaccount` as 2
                    mysql_query("UPDATE `orders` SET `brand`='sv', `account_id` = '{$accountinfo['id']}', `noaccount` = '2' WHERE `id` = '{$info['id']}' LIMIT 1");

                    //UPDATE THE CARD ASSOCIATED WITH ORDER WITH ACCOUNT ID ~~~
                   // mysql_query("UPDATE `card_details` SET `account_id` = '{$accountinfo['id']}' WHERE `order_session` = '{$info['order_session']}'");
                    ...
                    $subject = 'Superviral - Log Back In';
                    ...
                    <p>We can see you were unable to log back in.</p><br>
        
                    <p>Please click on the below link to login with one-click:</p><br>
                   
                    <p><a style="color: #2e00f4;
    ...
    margin: 5px auto;
    font-weight: 700;
    text-align:center;" href="https://superviral.io/'.$loclinkforward.'login/?accessToken=' . $token . '&orderlogin=true" target="_blank">Login with One-Click</a></p><br>
                    ...
                    <p>Superviral Team</p>';
                     ...
                     $tpl = str_replace('<a href="https://superviral.io/unsubscribe.php?unsub=now&id={md5unsub}">Unsubscribe</a>', '', $tpl);
                     ...
                     require($_SERVER['DOCUMENT_ROOT']. '/crons/emailer.php');
                     ...
                     if($emailSentTime == 0){
                        ...
                        mysql_query("UPDATE accounts SET password_email_sent='$now' WHERE id = $accountId");

                        emailnow($to,'Superviral','support@superviral.io',$subject,$emailTpl);
                    ...
                    $loginMsg = 'Incorrect password. We\'ve sent an email to: <b>'.$to.'</b>. Log back in <b>with one click</b>!'; 
...
$form = '<form method="POST">
     ...
     <a  target="BLANK" href="/'.$loclinkforward.'forgot-password/" class="btn btn3">I Forgot My Password</a>
      ...
      sendCloudwatchData('Superviral', 'existing-user-popup', 'OrderFinish', 'existing-user-relogin-function', 1);
  ...
  if(!empty($_POST['submit'])){
          ...
          if(!empty($_POST['password'])){
            ...
            // if email not exist then create new user
                     ...
                     // set plus_id cookie
                     setcookie("plus_id", $plus_id_value1, time() + (86400 * 360), "/","superviral.io"); // 86400 = 1 day
                     // set plus_token cookie
                     setcookie("plus_token", $plus_id_value2, time() + (86400 * 360), "/","superviral.io"); // 86400 = 1 day              
                     ...
                     //UPDATE THIS ORDER WITH ACCOUNT ID AND `noaccount` as 2
                     mysql_query("UPDATE `orders` SET `brand`='sv', `account_id` = '$newaccountid', `noaccount` = '2' WHERE `id` = '{$info['id']}' LIMIT 1");

                     //UPDATE THE CARD ASSOCIATED WITH ORDER WITH ACCOUNT ID ~~~
                     //mysql_query("UPDATE `card_details` SET `account_id` = '$newaccountid' WHERE `order_session` = '{$info['order_session']}'");
                     sendCloudwatchData('Superviral', 'new-user-popup', 'OrderFinish', 'new-user-register-function', 1);
...
$tpl = str_replace('{order_session}',$order_session,$tpl);
