...
if(empty($get)){
	...
	//	curl_setopt($curl, CURLOPT_PROXY, 'us.smartproxy.com:10000');
		...
		if(empty($get)){
			sendCloudwatchData('Tikoid', 'curl-image-load-failure', 'DownloadThumbs', 'curl-image-load-failure-function', 1);
		...
		if (!empty($get)) {
			...
			echo "putobject";
			if($putobject){
				sendCloudwatchData('Tikoid', 's3-image-upload-success', 'DownloadThumbs', 's3-image-upload-success-function', 1);
			}else{
				sendCloudwatchData('Tikoid', 's3-image-upload-failure', 'DownloadThumbs', 's3-image-upload-failure-function', 1);
