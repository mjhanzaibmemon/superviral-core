...
$tpl = file_get_contents('buy-tiktok-views.html');

$q = mysql_query("SELECT * FROM `packages` WHERE `type` = 'views' AND `brand` = 'to' ORDER BY price ASC");
...
$packages .= '			<div class="item dshadow">
				              ...
				              <div class="buyctn"><a title="'.$info['amount'].' followers" class="btn dshadow color4" href="/order/choose/'.$info['id'].'">Buy Now</a></div>
			            </div>';

}


$q = mysql_query("SELECT * FROM `reviews` WHERE `type` = 'views' AND `approved` = '1' AND `brand` = 'to' ORDER BY `id` DESC");
...
$contentq = mysql_query("SELECT * FROM `content` WHERE `country` = '{$locas[$loc]['sdb']}' AND `page` IN ('buy-tiktok-views', 'global') AND brand = 'to' ");
