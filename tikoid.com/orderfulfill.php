...
mysql_query('INSERT INTO supplier_cost SET `type` = "followers", `amount` = "'.$packageinfo['amount'].'", `service_id` = "'.$packageinfo[$japid].'", `cost` ="'. $supplier_cost .'", `page` = "admin/orderfulfill.php", `timestamp` = '.time().', `socialmedia` = "'.$packageinfo['socialmedia'].'", `brand` = "'.$packageinfo['brand'].'"');
}
...
if(($orderid)&&($info['packageid']!=='18')&&($info['packageid']!=='20')){mysql_query("UPDATE `orders` SET `done` = '1',`fulfill_id` = '$orderid' WHERE `order_session` = '{$info['order_session']}' ORDER BY `id` DESC LIMIT 1");}
