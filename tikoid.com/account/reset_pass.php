...
if((empty($_GET['id'])) || (empty($_GET['key']))){
	...
	header("Location: /".$locas[$loc]['account']."/".$locas[$loc]['login']."/"); //NO ID OR KEY SET
	die;
...
if((!empty($_GET['id'])) && (!empty($_GET['key']))){
  ...
  $q = mysql_query("SELECT * FROM `accounts` WHERE `email_hash` ='$id' and `resetpwstring` = '$key' AND `brand` = 'to' LIMIT 1");
  $num_rows = mysql_num_rows($q);
	...
	if($num_rows==0){//ALL CHECKS DONE if its not equal to one, then redirect
	 	header("Location: /".$locas[$loc]['login']."/?password=changedordone"); //NO MATCH FOUND
	  	die;
	}
	...
	if (time() - $info['resetpwtime'] > 2 * 3600) {
		...
		$expired=1;
		echo 'Expired';
		header("Location: /".$locas[$loc]['forgotpassword']."/?password=expired");
		die;
...
if(!empty($_POST['submitted'])){
	...
	if($newpassword==$confirmpassword){//Confirmed that both password's match, now time to log user in with password change notification
		...
		$updateaccountq = mysql_query("UPDATE `accounts`
			...
			WHERE `email_hash` = '$id' AND `brand` = 'to' LIMIT 1");
		...
		if($updateaccountq){
			...
			header('Location: /account/orders/?passwordchange=true');
...
$tpl = str_replace('{formsubmit}', $locas[$loc]['resetpassword'], $tpl);
