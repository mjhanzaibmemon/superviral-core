...
if($_GET['billingsave']=='true'){mysql_query("UPDATE `accounts` SET `disablesavepayments` = '0' WHERE `id` = '{$userinfo['id']}' AND `brand` = 'to' LIMIT 1");
header('Location: /account/settings/billing/');
}


if($_GET['billingsave']=='false'){mysql_query("UPDATE `accounts` SET `disablesavepayments` = '1' WHERE `id` = '{$userinfo['id']}' AND `brand` = 'to' LIMIT 1");
header('Location: /account/settings/billing/');
}
...
if((isset($error1))||(isset($error2))||(isset($error3))||(isset($error0))){
	...
	$checkothercardsq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `approved` = '1' AND `brand` = 'to' ");
...
/*	echo $expiryunix.'<hr>';
	...
	if((empty($error1))&&(empty($error2))&&(empty($error3))&&(empty($error0))){
			...
			$insertcardq = mysql_query("INSERT INTO `card_details` SET 
				...
				`exphash` = '$exphash', 
				$primarycard
				`expiryunix` = '$expiryunix', `brand` = 'to'
				");
...
if(!empty($_GET['deleteid'])){
	...
	$fetchdeletecardq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$deleteid' AND `brand` = 'to'  LIMIT 1");

	$fetechdeletecardinfo = mysql_fetch_array($fetchdeletecardq);

	$deletecardq = mysql_query("DELETE FROM `card_details` WHERE 
			`account_id` = '{$userinfo['id']}' 
		AND `id` = '$deleteid' AND `brand` = 'to'
		LIMIT 1");

	if($deletecardq)$success1 = '<div class="emailsuccess" style="margin-bottom:15px;">Your card ending with '.decrypt($fetechdeletecardinfo['lastfourhash'],$lastfoursecretphrase).' was deleted successfully!</div>';
...
if(!empty($_GET['primaryid'])){
	...
	$updateallasnotprimaryq = mysql_query("UPDATE `card_details` SET `primarycard` = '0' WHERE 
			`account_id` = '{$userinfo['id']}' AND `brand` = 'to' ");

	if($updateallasnotprimaryq)$updatenewprimarycardq = mysql_query("UPDATE `card_details`
	SET `primarycard` = '1'
	WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$primaryid' AND `brand` = 'to' 
		LIMIT 1");

	$fetchprimarycardq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `id` = '$primaryid' AND `brand` = 'to' LIMIT 1");

	$fetechprimarycardinfo = mysql_fetch_array($fetchprimarycardq);


	if($updatenewprimarycardq)$success1 = '<div class="emailsuccess" style="margin-bottom:15px;">Your card ending with '.decrypt($fetechprimarycardinfo['lastfourhash'],$lastfoursecretphrase).' was updated as the primary card!</div>';
...
$findcardsq = mysql_query("SELECT * FROM `card_details` WHERE `account_id` = '{$userinfo['id']}' AND `approved` = '1' AND `brand` = 'to' ORDER BY `id` DESC");
...
if(mysql_num_rows($findcardsq)!==0){
	...
	while($cardinfo = mysql_fetch_array($findcardsq)){
		...
		if(!empty($cardinfo['card_brand'])){
			...
			if(($now <= $cardinfo['expiryunix']) && ($cardinfo['expiryunix'] <= $nowplus)){


				$datediff = $now - $cardinfo['expiryunix'];
				$calctime = round($datediff / (60 * 60 * 24));

				$expiredmsg = '<div class="expired expiring">Expiring in '.str_replace('-','',$calctime).' days</div>';}

			if($now > $cardinfo['expiryunix']){$expiredmsg = '<div class="expired">Expired</div>';$makeprimary = '';}

			 

	$cardresults .= '<div class="savedcards '.$primaryclass.' dshadow">'.$cardbrandset.'**** '.decrypt($cardinfo['lastfourhash'],$lastfoursecretphrase).$expiredmsg.$makeprimary.'<a onclick="return confirm(\'Are you sure you want to delete this card?\');" class="deletecard" href="?deleteid='.$cardinfo['id'].'"><img src="/imgs/x-mark.png"></a>
...
if(mysql_num_rows($findsubcriptonsq)!==0){
	...
	while($subsinfo = mysql_fetch_array($findsubcriptonsq)){
		...
		$fetchtransactionsq = mysql_query("SELECT * FROM `automatic_likes_billing` WHERE `auto_likes_id` = '{$subsinfo['id']}' AND `brand` = 'to' ");
...
$tpl = file_get_contents('billing.html');
