...
if(empty($username)){
	sendCloudwatchData('Tikoid', 'error-post-unavailable', 'GetTiktokPost', 'error-post-unavailable-function', 1);
...
if($datatype == 'dp'){
    ...
    sendCloudwatchData('Tikoid', 'supernova-tikok-api-getprofile', 'GetTiktokPost', 'supernova-tikok-api-getprofile-function', 1);
    ...
    if(empty($get)){
        sendCloudwatchData('Tikoid', 'tt-dp-load', 'OrderReview', 'order-review-tt-dp-failure-function', 1);
    }
    echo '<pre>';
    //dp save
...
sendCloudwatchData('Tikoid', 'supernova-tikok-api-userid', 'GetTiktokPost', 'supernova-tikok-api-userid-function', 1);
// mysql_query("UPDATE `admin_statistics` SET `metric` = `metric` + 1 WHERE `type` = 'supernova_api_tiktok_getpost_userid' AND `brand` = 'to' LIMIT 1");
...
$secUID = $get->data->uid;
    ...
    sendCloudwatchData('Tikoid', 'tt-api-stats-success', 'GetTiktokPost', 'tt-api-stats-success-function', 1);
...
function tikAPI($uid, $username){
	...
	// mysql_query("UPDATE `admin_statistics` SET `metric` = `metric` + 1 WHERE `type` = 'supernova_api_tiktok_getpost_posts' AND `brand` = 'to' LIMIT 1");
    sendCloudwatchData('Tikoid', 'supernova-tikok-api-posts', 'GetTiktokPost', 'supernova-tikok-api-posts-function', 1);
    ...
    if (count($postThumbLinks) > 0) {
        ...
        foreach ($postThumbLinks as $node) {
            ...
            if (mysql_num_rows($findpostfile) == 0) {
                ...
                mysql_query("INSERT INTO `tt_thumbs` SET `added_on_tiktok` = '$postTime', `shortcode` = '$code',thumb_url = '$postSrc', `ttusername` = '$username', `added` = '$now', media_type='video'");
                ...
                mysql_query("UPDATE `tt_thumbs` SET `added_on_tiktok` = '$postTime',`ttusername` = '$username' WHERE `shortcode` = '$code' LIMIT 1");
...
function Dp($profile, $dpimgname){
            ...
            if($putobject){
				...
				sendCloudwatchData('Tikoid', 's3-image-upload-success', 'GetTiktokPost', 's3-image-upload-success-function', 1);
			}else{
				sendCloudwatchData('Tikoid', 's3-image-upload-failure', 'GetTiktokPost', 's3-image-upload-failure-function', 1);
