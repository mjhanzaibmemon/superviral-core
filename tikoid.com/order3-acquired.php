...
if(empty($info['emailaddress']) || empty($info['igusername'])){
    ...
    header('Location: https://tikoid.com/order/details/');
...
if (array_key_exists('submitForm', $_POST)) {
    ...
    mysql_query("UPDATE `admin_statistics` SET `metric` = `metric` + 1 WHERE `type` = 'payment_attempts_per_hour' AND `brand` = 'to' LIMIT 1");
    sendCloudwatchData('Tikoid', 'payment-attempt', 'OrderPayment', 'payment-attempt-function', 1);

    sendCloudwatchData('Tikoid', 'payment-attempt-per-hour', 'AdminStats', 'payment-attempt-per-hour-function', 1);
...
$checkforfraudq = mysql_query("SELECT * FROM `blacklist` WHERE `emailaddress` 
                                    ...
                                    `ipaddress` = '{$info['ipaddress']}' AND `brand` = 'to' LIMIT 1");
...
if($userBlackList == 1){
    sendCloudwatchData('Tikoid', 'blacklist-payment-attempt', 'OrderPayment', 'blacklist-payment-attempt-function', 1);
}
...
if ($paymentdetailscheck == 1) {
    ...
    `added` = '$nowblacklist', `brand` = 'to', `source` = 'order-payment'");
...
if ((!empty($submit)) && ($paymentdetailscheck == 2)) {
    ...
    if ((!empty($pan)) && (is_numeric($pan))) {
        ...
        mysql_query("UPDATE `order_session` SET `lastfour` = '{$lastfour}' WHERE `id` = '{$info['id']}' AND `brand` = 'to' LIMIT 1");
 ...
 */
 ...
 $CommonErrorMsg = "";
   ...
   // get stats
   $statsQuery =  mysql_query("SELECT * FROM `admin_statistics` WHERE `type` = 'payment_attempts_per_hour' AND `brand` = 'to' LIMIT 1");   
   ...
   $submitBtn = ' <input id="submitbtnthis" type="submit" class="color4 btn" onclick="onSubmitData(event);" name="submitbtn" value="{maincta}">';
   $allowRecaptcha = true;
  }else{
   $submitBtn = ' <input id="submitbtnthis" type="submit" class="color4 btn" name="submitbtn" value="{maincta}">';
   $allowRecaptcha = false;
  }
       ...
       if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])){  
        ...
        if ($ValidateForm) {
            ...
            $verifyCardData = array(
                ...
                "company_mid_id" => $mid, // for testing acquired payment gateway
            ...
            <body onload=\"OnLoadEvent()\">
                ...
                $challenge_url = $siteDomain . '/order3-acquired-3dv2.php?redirectid='.$info['order_session'];
            ...
            if (in_array($response['response_code'], array(503))) { // For 3Dv2 return 503
                ...
                $termurl = $siteDomain . '/order3-acquired-3dv1.php?redirectid='.$info['order_session'];
            ...
            }else if(in_array($response['response_code'], array(251, 401, 403))){
                ...
                if($alreadyExist == 0){
                    ...
                    `added` = '$nowblacklist', `last_updated` = '$nowblacklist',  brand = 'to', `source` = 'order-payment'");
                    sendCloudwatchData('Tikoid', 'blacklist-insert', 'OrderPayment', 'blacklist-insert-function', 1);
                ...
                // Do on Success
                sendCloudwatchData('Tikoid', 'payment-made', 'OrderPayment', 'order-payment-form-function', 1);
...
if(!empty($CommonError) || !empty($CardNameError) || !empty($CardNumError) || !empty($CardDateError) || !empty($CardCvcError)){
    ...
    sendCloudwatchData('Tikoid', 'payment-form-error', 'OrderPayment', 'order-payment-form-error-function', 1);
...
$tpl = str_replace('{redirect}', 'https://tikoid.com/order/payment-processing/', $tpl);
...
sendCloudwatchData('Tikoid', 'order-payment', 'UserFunnel', 'user-funnel-order-payment-function', 1);
...
sendCloudwatchData('Tikoid', 'page-load-order-payment', 'PageLoadTiming', 'page-load-order-payment-function', number_format($execution_time_sec, 2));
