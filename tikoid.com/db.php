...
if(!empty($initial) && $initial != "tikoid.") $_SERVER['DOCUMENT_ROOT'] = $_SERVER['DOCUMENT_ROOT'] . $subdomain;
...
if ($dbMasked == 1 && $devstage == 'test') {
        ...
        $dbName = 'etra_tikoid_masked'; // masked
...
if($webhookbypass==0){
if (strpos($_SERVER['SERVER_NAME'], 'superviral.io') !== false){}else{die('Error');}//Protection from Nameserver imitation
}
...
if(($subdomainloc=='us')||($subdomainloc=='uk')){
        ...
        $_SERVER['REQUEST_URI'] = preg_replace('/'.preg_quote('%7B').'[\s\S]+?'.preg_quote('%7D').'/', '', $_SERVER['REQUEST_URI']);
        $_SERVER['REQUEST_URI'] = str_replace('%7B%7D','',$_SERVER['REQUEST_URI']);
        $cleanurl = str_replace('superviral.io//','superviral.io/','https://superviral.io'.$_SERVER['REQUEST_URI']);
...
if(($subdomainloc=='www')||($subdomainloc=='fr')||($subdomainloc=='es')||($subdomainloc=='it')||($subdomainloc=='fr')||($subdomainloc=='de')){
...
$cleanurl = str_replace('superviral.io//','superviral.io/','https://superviral.io'.$_SERVER['REQUEST_URI']);
        ...
        if ((strpos($_SERVER['REQUEST_URI'], '/uk/') !== false)||(strpos($_SERVER['REQUEST_URI'], '/us/') !== false)) {
            ...
            header('Location: '.$cleanurl,TRUE,301);die;
