...
if($_GET['message']=='updatetrue'){$message = '<div class="emailsuccess">Order ID #'.$mainid.' successfully updated with Supplier ID: '.$theid.'.</div>';
$search = $mainid;}



if($type=='cancelled')$q = mysql_query("SELECT * FROM `orders` WHERE `defect` = '2' AND `refund` = '0' ORDER BY `id` ASC LIMIT 1");
if($type=='partial')$q = mysql_query("SELECT * FROM `orders` WHERE `defect` = '3' AND `refund` = '0' ORDER BY `id` ASC LIMIT 1");

if(!empty($search)){$q = mysql_query("SELECT * FROM `orders` WHERE `id` = '$search' ORDER BY `id` ASC LIMIT 1");}
...
foreach($balance as $key => $order){
    ...
    if($order=='Incorrect request')continue;

    if($order['status']=='Partial')$left = ' - '.($info['amount'] - $order['remains']).'/'.$info['amount'];

	if(!empty($order))$thisorderstatus .= '<a target="_BLANK" rel="noopener noreferrer" href="'.$fulfillmentsite.'/orders?search='.$key.'">'.$key.'</a> - '.$order['status'].$left.'<br>';
...
$notesadmin = '<div class="sdiv">'.$notesadmin.'</div>';
    ...
    <div class="sdiv">
                        <form id="makeorder" action="ordermakefordefect.php" method="POST">
...
<div style="margin-bottom:30px;"><form method="POST" action="https://superviral.io/admin/defectorders.php?&type=defect">
...
<?=$styles?>
	...
	<body onload="init();">
		...
		<div class="adminmenu">
			<a href="?type=cancelled">Cancelled orders</a>
			<a href="?type=partial">Partial orders</a>
		</div>
