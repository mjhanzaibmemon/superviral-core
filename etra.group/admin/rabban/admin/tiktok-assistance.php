...
$q = mysql_query("SELECT * FROM `admin_notifications` WHERE `done` = '1' ORDER BY `response` ASC LIMIT 3");
...
while($info = mysql_fetch_array($q)){
			...
			$msgs2 .= '
				...
				<tr>
				<td>'.$info['emailaddress'].'<br>#️⃣ Order ID: <a target="_BLANK" href="https://superviral.io/admin/tiktok-check-user.php?orderid='.$info['orderid'].'#order'.$info['orderid'].'">#'.$info['orderid'].'</a></td>
				<td>
