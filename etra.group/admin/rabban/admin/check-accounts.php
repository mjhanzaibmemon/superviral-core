...
if (!empty($deleteAccountID)) {
    mysql_query("DELETE FROM `accounts` WHERE `id` = '$deleteAccountID' LIMIT 1"); 
    $deleteMessage = '<div class="emailsuccess emailfailed">Deleted Successfully</div>';
}

if (!empty($user)) {$q = mysql_query("SELECT * FROM `accounts` WHERE `email` LIKE '%$user%' OR `username` LIKE '%$user%' ORDER BY `id` DESC");
    ...
    while ($info = mysql_fetch_array($q)) {
        ...
        $results .= '
            ...
            <table id="account' . $info['id'] . '" class="perorder">
                ...
                <tr><td>📧 Email address: </td><td><a href=' . $siteDomain . '/admin/check-accounts.php?user=' . $info['email'] . '>' . $info['email'] . '</a></td></tr>
                ...
                <tr><td></td><td><form href="' . $siteDomain . '/admin/check-accounts.php" method ="POST"><input type="hidden" name="deleteaccountid" value="'. $info['id'] .'"><button class="btn btn3 report"  onclick="return confirm(\'Are you sure you want to delete this account?\');" >Delete Account</button></form></td></tr>
            </table>
...
<?=$styles?>
    ...
    <body>
        ...
        <h1 style="text-align:center;margin-top:35px;">Check Accounts 📦</h1>
        ...
        <div class="box23">
            ...
            <table class="articles">
                ...
                <tr>
                    ...
                    <td>👨👩 Account:<span class="searchspan">Search by:<br>- Username<br>- Email address</span></td>
                ...
                <tr>
                    ...
                    <td></td>
                    <td><input style="float:left;" type="submit" name="submit" class="btn color3" value="Search"><a href='<?=$siteDomain?>/admin/check-accounts.php' class="btn btn3 report" style="float:right;">Reset Search</a>
