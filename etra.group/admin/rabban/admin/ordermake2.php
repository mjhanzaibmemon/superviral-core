...
if($freeorder==0){//IF IT ISNT A FREE ORDER THEN CONTINUE AS USUAL
		...
		// var_dump($ordersession);echo '<br>';
		// var_dump($choosepostsql);echo '<br>';
		// var_dump($multiamountposts);echo '<br>';
		// var_dump($info['chooseposts']);echo '<br>';
		// var_dump($chooseposts);echo '<br>';
		// var_dump($posts1);echo '<br>';
		// var_dump($posts2);echo '<br>';
...
if(!empty($orderid)){
	...
	if($defectpagefrom=='defect'){
		...
		header('Location: defectorders.php?type='.$pagefrom.'&message=updatetrue&theid='.$orderid.$noorderstate);
		die;
	...
	if($update=='nodefect'){
		...
		header('Location: '.$defectpagefrom.'orders.php?type='.$pagefrom.'&message=updatetrue&theid='.$orderid.$noorderstate);
		die;

	}else{

		header('Location: '.$defectpagefrom.'orders.php?type='.$pagefrom.'&message=updatetrue2&theid='.$orderid123.$noorderstate);
	}
	die;
