...
if(empty($type))header('Location: https://superviral.io/admin/tiktok-orders.php?type=missing');
...
//////////////////////////////////////////////
    ...
    $respondedby = $_SERVER['PHP_AUTH_USER'];
    $response = time();
    ...
    if(!empty($gotopage))header('Location: https://superviral.io/admin/tiktok-orders.php?type=reported&page='.$gotopage);

}

if($type=='missing'){$q = mysql_query("SELECT * FROM `orders` WHERE `id` > '73663' AND `fulfill_id` = '' AND `defect` = '0' AND `refund` = '0' AND `disputed` = '0' ORDER BY `packagetype` ASC LIMIT 100");}
...
$i++;
               ...
               $supplierserviceid = $sserviceid['jap1'];
            	...
            	$articles .= '<tr>
            					...
            					<td class="first'.$info['id'].'">'.$fraudwarning.'<b> '.$i.'. <a target="_BLANK" href="https://superviral.io/admin/tiktok-check-user.php?orderid='.$info['id'].'#order'.$info['id'].'">'.$info['id'].'</a> - '.$show.'</b><br>£'.$info['price'].' - '.$info['amount'].' '.$info['packagetype'].'<br>'.date('l jS \of F Y H:i:s ', $info['added']).'<br>
                                <a target="_BLANK" href="https://superviral.io/admin/tiktok-check-user.php?user='.$info['emailaddress'].'">'.$info['emailaddress'].'</a>
                                <br>
                                '.$payment_id.'<br>


            						<form action="tiktok-ordermake'.$freefollowers.'.php" method="POST">
                                    ...
                                    <form method="POST" action="tiktok-ordersupdate.php">
                    <input type="hidden" name="pagefrom" value="'.$type.'">
                    ...
                    <input type="submit" onclick="return confirm(\'Are you sure you want to ignore this order?\');" class="btn btn3 report copy-buttonn nlbtn" value="Ignore Defect" style="float:right;">
                                ...
                                <form method="POST" action="tiktok-ordersupdate">
                                <input type="hidden" name="pagefrom" value="'.$type.'">
                                <input type="hidden" name="update" value="save">
                                <input type="hidden" name="id" value="'.$info['id'].'">
...
if($type=='reported'){//REPORTED
   ...
   if (strpos($info['orderid'], 'AL') !== false){
        ...
        $hrefurlcheck = 'https://superviral.io/admin/tiktok-check-user.php';
	...
	$articles .='
    <tr>
	<td>Customer Order ID: <a target="_BLANK" href="'.$hrefurlcheck.'?orderid='.$info['orderid'].'">'.$info['orderid'].'</a><br><a target="_BLANK" href="https://superviral.io/admin/tiktok-check-user.php?user='.$info['emailaddress'].'">'.$info['emailaddress'].'</a>
...
<div style="margin-bottom:30px;"><form method="GET" action="https://superviral.io/admin/tiktok-orders.php?&type=defect">
