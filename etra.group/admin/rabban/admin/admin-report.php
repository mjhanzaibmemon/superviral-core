...
$info['emailaddress'] = trim($info['emailaddress']);

$getreportedinfoq = mysql_query("SELECT * FROM `orders` WHERE `emailaddress` LIKE '%{$info['emailaddress']}%' order by id desc LIMIT 10");
...
if(!empty($getreportedinfo)){
    ...
    while($orderInfo = mysql_fetch_array($getreportedinfoq)){
        ...
        $orderHistory .= '<tr><td>#' . $orderInfo['id'] . '</td><td id="statusBind'.$l.'"></td><td id="startCountBind'.$l.'">0</td><td id="fulfillmentID'.$l.'"></td></tr>';
        $orderArr[] = $orderInfo['id'];
        $l++;
...
<body>
    ...
    min-height: 0px !important;
    width: 0px;
    resize: none;"><?= $info['fulfill_id'] ?></textarea>
                    <a href="#0" style="margin: 0px;margin-top: -15px !IMPORTANT;width:80px;margin-bottom: -13px;    margin-right: 10px;" class="btn btn3 copy-button">Copy #</a>
                    ...
                    <table class="articles">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="background: white;   border-right: none;">Fulfill ID</td>
                            </tr>
        ...
        $.ajax({	
            ...
            success: function(response){ 
                ...
                for (i =0; i<response.status.length;i++){
                    ...
                    $('#fulfillmentID'+(parseInt(i)+1)).html(response.fulfillmentID[i]);
