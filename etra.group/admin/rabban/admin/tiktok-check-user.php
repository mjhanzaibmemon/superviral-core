...
$thisstaffmember = addslashes($_SERVER['PHP_AUTH_USER']);
...
if(!empty($user)){$q = mysql_query("SELECT * FROM `orders` WHERE `emailaddress` LIKE '%$user%' OR `igusername` LIKE '%$user%' OR `payment_id` LIKE '%$user%' OR `payment_id_desc` LIKE '%$user%' OR `payment_id_desc` LIKE '%$user%' ORDER BY `id` DESC");$field = '<input type="hidden" name="user" value="'.$user.'">';}
if(!empty($orderid)){
...
while($info = mysql_fetch_array($q)){
			...
			$adminnotifsq = mysql_query("SELECT * FROM `admin_notifications` WHERE `orderid` = '{$info['id']}' LIMIT 12");	
    ...
    font-style: italic;"> (reported by '.ucfirst($adminnotifinfo['admin_name']).' -  '.ago($adminnotifinfo['added']).', '.date("d/m/Y H:i:s",$adminnotifinfo['added']).')</span>
				...
				<form action="https://superviral.io/admin/tiktok-check-user.php#order'.$info['id'].'" method="POST">'.$fields.'
		...
		//	echo $saleamount.' - '.$salecost;;
			...
			if(($info['packagetype']=='followers')||($info['packagetype']=='views')){$arextendsentence = 'We can also split the 1000 likes to multiple IG posts.';}

			///// AUTO RESPONSES

			$autoresponses1 = '<div class="foo" >
<textarea class="language-less">
First and foremost, thank you very much for choosing Tikoid for upgrading your profile – it means a lot to us! In regards to the order number associated with your email address, I’ve been able to locate a specific order.

I can see that your order is for '.$info['amount'].' '.$info['packagetype'].' and is '.$arstatus.'.'.$artime.' Bear in mind at Tikoid, from fulfilling thousands of orders since 2012, we’ve gained a tremendous amount of experience. This has allowed us to develop the safest methods for delivery so that you can maintain as many '.$info['packagetype'].' as possible - ensuring maximum customer satisfaction.

If your order has not started yet at all, then our systems are still determining the right delivery method and the right type of '.$info['packagetype'].' based on your TikTok profile’s metrics.

From what our previous customers have experienced, please stay away from IG service providers that claim to provide instant deliveries without caring about their customer’s TikTok account. These illicit companies can usually lead to shadow bans from TikTok or even a ban.
...
<textarea class="language-less">
Thank you for contacting Tikoid Customer Care. First and foremost, I would like to apologise for the experience you’ve had on Tikoid and can only imagine the inconvenience you may have experienced.

Please bear in mind that _________ (reason why it’s not Tikoid’s fault – to protect our brand), that being said we’ve reported this on to our technical team to look into so that this issue doesn’t happen again.
...
At Tikoid, the customer always comes first and we hope to resolve this issue ASAP.
...
We have refunded the full amount of £'.$price.' to the card ending with '.$info['lastfour'].' back to your account, and as a goodwill gesture, you can keep the '.$info['packagetype'].' you received from Tikoid. Please allow 5-10 days for the refund to appear on your bank statement.

Thank you for using Tikoid and once again we do apologise for the inconvenience this may have caused you. We look forward to serving you again in the future.
</textarea>
...
<textarea class="language-less">
Thank you for contacting Tikoid Customer Care. I am extremely sorry to hear that you have experienced this issue. We\'ve looked into your order and it seems it was delivered. 

Also to add to that, your order is for TikTok followers. At Tikoid, we provide you with Tikoid Refill – where our system automatically scans your TikTok account for any followers/likes (that you’ve ordered) that has been lost. The system will then automatically reimburse your account with followers and likes to ensure you get what you ordered.

Also in some cases, Tikoid may send you an email/text notification, and you may see that your order is complete but you haven’t received your order. For example, you’ve ordered 1000 followers and receive an email notification saying “your order is complete”. But you check your TikTok account and it has increased by only 700 followers.

This happens when TikTok removes your followers in between the time we’ve told you that the order is complete and the time you’ve checked your account. But not to worry, with our Tikoid Refill – we’ll keep refilling your account’s followers/likes according to what you’ve ordered within 30-days of your order.
...
<button class="btn btn3 report copy-button">Copy Has Received Order</button>
	...
	$autoresponses4 = '<div class="foo" >
<textarea class="language-less">
Thank you for contacting Tikoid Customer Care. I am extremely sorry to hear that you have experienced this issue. We\'ve looked into your order and it seems it was delivered. If this issue still persists, please do not hesitate to contact us again.
...
if(($_SERVER['PHP_AUTH_USER']=='rabban')||($_SERVER['PHP_AUTH_USER']=='admin')){

$offerfreefollowers = '<tr class="grey"><td>Offer Free Followers: </td><td><a target="_BLANK" href="https://superviral.io/admin/tiktok-offer-free-followers.php?id='.$info['id'].'" class="btn btn3 report">Offer Followers</a>
<a target="_BLANK" href="https://superviral.io/admin/tiktok-offer-free-likes.php?id='.$info['id'].'" class="btn btn3 report">Offer Likes</a></td></tr>';

$supplierfulfillid = '<tr class="grey"><td>Supplier Fulfill ID: </td><td>'.$thisorderstatus.'</td></tr>';
$supplierfulfillidtd = $thisorderstatus;

if($_SERVER['PHP_AUTH_USER']=='rabban'){
...
{$refundavail = '<form action="tiktok-refundorder.php?" method="POST">
...
if($info['refundtime']!=='0')$refunddate = '<tr><td>Refund issued on : </td><td>'.date('l jS \of F Y H:i:s ', $info['refundtime']).'</td></tr>';
			...
			<table id="order'.$info['id'].'" class="perorder">
				...
				<tr><td>Tikoid Order #'.$info['id'].'</td><td></td></tr>
				<tr><td>#️⃣ Order ID: </td><td>'.$info['id'].'</td></tr>
				<tr><td>📧 Email address: </td><td><a href="https://superviral.io/admin/tiktok-check-user.php?user='.$info['emailaddress'].'">'.$info['emailaddress'].'</a></td></tr>
				<tr><td>📦 Package: </td><td>'.$packagetype.'</td></tr>
				<tr><td>💸 Amount Paid: </td><td>£'.$price.$refunded.'</td></tr>
				<tr><td>🧍 TikTok username: </td><td><a target="_BLANK" rel="noopener noreferrer" href="https://instagram.com/@'.$info['igusername'].'/">'.$info['igusername'].'</a></td></tr>
				'.$chooseposts.'
				...
				'.$trackinginformation.'
				<tr><td>Tracking page: </td><td><a target="_BLANK" href="https://tikoid.com/track-my-order/'.$info['order_session'].'">'.$info['order_session'].'</a></td></tr>
				<tr><td>❤️😍 Refund offers: </td><td>'.$refundoffers.'</td></tr>
				<tr><td>📝 Report: </td><td>'.$reportnotifs.'
				<form action="https://superviral.io/admin/tiktok-check-user.php#order'.$info['id'].'" method="POST">'.$submittedfield.$field.'
...
.perorder{width:100%;}
.perorder tr:first-child td{background-color:#ff93bd;font-weight: bold;font-size:20px;}
...
copyTextareaBtn.addEventListener('click', function(event) {
  ...
  try {
	...
	<body>
		...
		<h1 style="text-align:center;margin-top:35px;"><?=$svgtiktokh1?> Check Tikoid User 	</h1>
		...
		<div class="box23">
			...
			<table class="articles">
				...
				<tr>
					...
					<td></td>
					<td><input style="float:left;" type="submit" name="submit" class="btn color3" value="Search"><a href="https://superviral.io/admin/tiktok-check-user.php" class="btn btn3 report" style="float:right;">Reset Search</a>
					<a href="https://superviral.io/admin/tiktok-no-order-report.php" class="btn btn3 report" style="float:right;">Submit No-Order Report</a></td>
