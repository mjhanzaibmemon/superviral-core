...
if (!empty($initial) && $initial != "etra.") $_SERVER['DOCUMENT_ROOT'] .= $subdomain;
...
if (isset($_GET['id'])) {
    ...
    if ($row = mysql_fetch_array($articleQ)) {
        ...
        if (
            stripos($article_title, 'tiktok') !== false
        ) {
...
$total = count($ids);
$redirectUrl = '/admin/articles/new/images/generate_tn.php?ids=' . urlencode(implode(',', $ids)) . '&edit=';
