...
$tpl = file_get_contents('editArticle2/index.html');
...
if ($id == 'new') {
    ...
    $q = mysql_query("INSERT INTO `articles` SET shortdesc='', `article` = '',`published`='1', brand='".addslashes($brand)."', `added_by`='" .addslashes($_SESSION['first_name']). "', `article_type`='private'");

    $newid = mysql_insert_id();

    if ($q) {
        header('Location: editarticle.php?id=' . $newid . '&message=3');
    } else {
...
if ($_POST['delete'] == 'Delete') {
    ...
    $q = mysql_query("DELETE FROM `articles` WHERE `id` = '$id' AND brand = '$brand' LIMIT 1");

    if ($q) {
        header('Location: /admin/articles/?&message=1');
        die;
    }
}
...
if (isset($_GET['aiid']) && !empty($_GET['aiid'])) {
    ...
    if ($q) {
        header('Location: editarticle.php?id=' . $id . '&message=5');
        die;
    }
}
...
if ($_POST['save'] == 'Save') {
	...
	`added_by` = '".addslashes($_SESSION['first_name'])."',
    ...
    $qq .= " `brand` = '".addslashes($brand)."' WHERE `id` = '$id' LIMIT 1";
    ...
    if ($q) {
        ...
        if ($currentIndex !== false && isset($idList[$currentIndex + 1])) {
            ...
            header('Location: editarticle.php?id=' . $nextId . '&ids=' . implode(',', $idList));
        } else {
            // echo '<script>alert(\'All done successfully.\');</script>';
            header('Location: /admin/articles/');
        }

        // header('Location: editarticle.php?id=' . $id . '&message=1');
        die;
    }
}
...
if ($_POST['submit'] == 'Publish') {
    ...
    if ($q) {
        header('Location: editarticle.php?id=' . $id . '&message=2');
        die;
    }
}
...
if ($info['superadmin_approve'] == '1' && $_SESSION['first_name'] != 'rabban' && $_SESSION['first_name'] != 'mac' && $_SESSION['first_name'] != 'tahmin' && $_SESSION['first_name'] != 'tahmin.a') {
