...
die('this is not the live version right now, use version 2 instead');
...
$q2 = mysql_query("SELECT * FROM `users` WHERE `source` = 'order' AND `locked` = 1 AND `unsubscribe` = '0' AND `monthlyfreefollowers` = '0' ORDER BY `id` DESC");

$q = mysql_query("SELECT * FROM `users` WHERE `source` = 'order' AND `locked` = 1 AND `unsubscribe` = '0' AND `monthlyfreefollowers` = '0' ORDER BY `id` DESC LIMIT 25");

if (mysql_num_rows($q) == '0') {

    $q2 = mysql_query("SELECT * FROM `users` WHERE `source` = 'cart' AND `locked` = 1 AND `unsubscribe` = '0' AND `monthlyfreefollowers` = '0' ORDER BY `id` DESC");

    $q = mysql_query("SELECT * FROM `users` WHERE `source` = 'cart' AND `locked` = 1 AND `unsubscribe` = '0' AND `monthlyfreefollowers` = '0' ORDER BY `id` DESC LIMIT 25");
    echo 'CART MODE<hr>';
}
...
while ($info = mysql_fetch_array($q)) {
    ...
    if (!empty($searchorders['igusername'])) {
        $subject = '@' . ucfirst($searchorders['igusername']) . ': Get 30 '. $product .' Followers! (One Click)';
        // $subject = '@'.ucfirst($searchorders['igusername']).': Black Friday - 30 '. $product .' Followers!';
        $subject = '@' . ucfirst($searchorders['igusername']) . ': Your 30 '. $product .' Followers';
    ...
    /*        if(!empty($searchorders['igusername'])){
            $subject = '@'.$searchorders['igusername'].': Black Friday! Get 30 '. $product .' Followers! (One Click)';
            $igusernameqinsert = "`username` = '{$searchorders['igusername']}',";
        }
        else{
            $subject = 'Black Friday! Get 30 Free '. $product .' Followers Now!';
    ...
    */
    ...
    echo "<div style='width:600px;float:left;padding:5px;border:1px solid #000'><b>$o</b><br>
        ...
        src='/crons/monthlyffwindows3.php?&email={$email}&loc2={$loc2}&subject={$subject}&freetrialmd5={$freetrialmd5}&tikoidfreetrialmd5={$freetrialmd5}&md5unsub={$md5unsub}&source={$source}&brand={$brand}&country={$country}{$accountquerylogin}'></iframe></div>";
