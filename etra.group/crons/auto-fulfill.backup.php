...
while ($info = mysql_fetch_array($q)) {
    ...
    $supplierError = $order1 -> error;
    if(!empty($supplierError)){
        //$updateq =  mysql_query("UPDATE `orders` SET `supplier_errors` = '$supplierError' WHERE `id` = '{$info['id']}' ORDER BY `id` DESC LIMIT 1");
    }
