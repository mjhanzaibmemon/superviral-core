...
$q2 = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'order' AND `monthlyfreelikes` = '0' AND `unsubscribe` = '0' ORDER BY `id` DESC");

$q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'order' AND `monthlyfreelikes` = '0' AND `unsubscribe` = '0' ORDER BY `id` DESC LIMIT 25");

if (mysql_num_rows($q) == '0') {

    $q2 = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'cart'   AND `monthlyfreelikes` = '0' AND `unsubscribe` = '0' ORDER BY `id` DESC");

    $q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'cart'   AND `monthlyfreelikes` = '0' AND `unsubscribe` = '0' ORDER BY `id` DESC LIMIT 25");
    echo 'CART MODE<hr>';
}
...
while ($info = mysql_fetch_array($q)) {
    ...
    if (!empty($searchorders['igusername'])) {
        $subject = '@' . ucfirst($searchorders['igusername']) . ': Your Instagram Likes';
    ...
    echo "<div style='width:600px;float:left;padding:5px;border:1px solid #000'><b>$o</b><br>
        ...
        src='/crons/monthlyflwindows.php?&email={$email}&loc2={$loc2}&subject={$subject}&freetrialmd5={$freetrialmd5}&tikoidfreetrialmd5={$freetrialmd5}&md5unsub={$md5unsub}&source={$source}&country={$country}{$accountquerylogin}'></iframe></div>";
