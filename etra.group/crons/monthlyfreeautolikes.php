...
$q2 = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'order' AND `locked` = 1 AND `monthlyfreeautolikes` = '0' ORDER BY `id` DESC");

//$q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'order' AND `locked` = 1 AND `monthlyfreeautolikes` = '0' ORDER BY `id` DESC LIMIT 1");
$q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'order' AND `locked` = 1 AND `monthlyfreeautolikes` = '0' AND emailaddress='mac@etra.group' ORDER BY `id` DESC LIMIT 1");

if (mysql_num_rows($q) == '0') {

    $q2 = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'cart' AND `locked` = 1  AND `monthlyfreeautolikes` = '0' ORDER BY `id` DESC");

    //$q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'cart' AND `locked` = 1  AND `monthlyfreeautolikes` = '0' ORDER BY `id` DESC LIMIT 1");
    $q = mysql_query("SELECT * FROM `users` WHERE `brand` = 'sv' AND `source` = 'cart' AND `locked` = 1  AND `monthlyfreeautolikes` = '0' AND emailaddress='mac@etra.group' ORDER BY `id` DESC LIMIT 1");
    echo 'CART MODE<hr>';
}
...
while ($info = mysql_fetch_array($q)) {
    ...
    //mysql_query("UPDATE `users` SET `monthlyfreeautolikes` = '1' WHERE `id` = '{$info['id']}' LIMIT 1");
    ...
    if (!empty($searchorders['igusername'])) {
        $subject = '@' . ucfirst($searchorders['igusername']) . ': Get Free Auto Instagram Likes! (One Click)';
        // $subject = '@'.ucfirst($searchorders['igusername']).': Black Friday - Free Auto Instagram Likes!';
        $igusernameqinsert = "`username` = '{$searchorders['igusername']}',";
    } else {
        $subject = 'Black Friday - Get Free Auto Instagram Likes Now!';
        //$subject = 'Get Free Auto Instagram Likes Now!';
        //$subject = '2022 NEW YEAR! - Free Auto Instagram Likes!';
    }
    ...
    echo "<div style='width:600px;float:left;padding:5px;border:1px solid #000'><b>$o</b><br>
        ...
        src='/crons/monthlyfalwindows.php?&email={$email}&loc2={$loc2}&subject={$subject}&freetrialmd5={$freetrialmd5}&tikoidfreetrialmd5={$freetrialmd5}&md5unsub={$md5unsub}&source={$source}&country={$country}{$accountquerylogin}'></iframe></div>";
